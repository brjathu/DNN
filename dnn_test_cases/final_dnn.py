...
def Inference(filePath, X_valid):
    ...
    with tf.Session(config=config) as sess:
        ...
        predict_fc = sess.graph.get_tensor_by_name("prefix/hidden_layer6/Tanh:0")
...
def Inference_2(filePath, X_valid):
    ...
    with tf.Session(config=config) as sess:
        ...
        pred_softmax = sess.graph.get_tensor_by_name("softmax:0")
        # predict_fc = sess.graph.get_tensor_by_name("prefix/rnn/multi_rnn_cell_29/cell_1/dropout/mul:0")
        predict_fc = sess.graph.get_tensor_by_name("hidden_layer6/Tanh:0")
...
main_dir = "/flush1/raj034/DNN/100EPOCH/sniff/"

hidden_unit_folder = [64]
for h_folder in hidden_unit_folder:
    for layer in [2]:
        new_path = main_dir + str(h_folder) + "_tanh/99/"
        pb_location = "/flush1/raj034/DNN/final/dnn/sniff/"
            ...
            for quant in ["quant_yes"]:
                ...
                for lvl in [100, 80, 60, 40, 20, 10, 5]:
                    ...
                    if(quant == "quant_yes"):
                        ...
                        # # accuracy test
                        # predictions_frozen = []
                        # predictions_frozen, vect_out = Inference(main_location + "/pb/DNN_" + breath + "_" + quant + "_fact_" + str(lvl) + ".pb", x_test)
                        # os.system("zip -r " + main_location + "/pb/compressed.zip " + main_location + "/pb/DNN_" + breath + "_" + quant + "_fact_" + str(lvl) + ".pb")
                        # logEntry(os.system("ls -l " + main_location + "/pb/compressed.zip"))
                        # plotConfusionMatrix(list(predictions_frozen), y_test, 100, main_location, "confusion_matrix.png", "DNN - " + breath + "_" + quant)
