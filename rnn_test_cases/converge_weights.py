...
parser.add_argument('--save_location', help='save the compressed file')
...
parser.add_argument('--min_n_weights', default=256, type=int,
                    help='The minimum amount of values in an Const for it to be compressed (helps filter consts that aren\'t weights, only applies to global clustering)')
...
if __name__ == "__main__":
    ...
    new_graph_def = converge_weights(graph_def, whitelisted=args.whitelisted.split(
        ","), global_clusters=args.global_clusters, n_clusters=args.n_clusters, min_n_weights=args.min_n_weights)

    tf.train.write_graph(new_graph_def, '.', args.save_location, as_text=False)
