...
def Inference(filePath, X_valid):
    ...
    with tf.Session(config=config) as sess:
        ...
        for x in range(0, X_valid.shape[0]):
            ...
            tmp, vect_128 = sess.run([pred_softmax, predict_fc], feed_dict={X: tmp})
...
tests = os.listdir("/flush1/raj034/RNN/model/test_cases/" + breath + "/" + quant + "/")
tests.sort()
...
# X_valid = np.load("/home/brjathu/Documents/CSIRO/RNN/DNN/data/x_test_old.npy")
# y_valid = np.load("/home/brjathu/Documents/CSIRO/RNN/DNN/data/y_test_old.npy")
logEntry(np.ndim(X_valid))
logEntry(np.ndim(y_valid))

for cases in tests:
    path = "/flush1/raj034/RNN/model/test_cases/" + breath + "/" + quant + "/" + str(cases)
    logEntry(path)
    logEntry(strftime("%Y-%m-%d %H:%M:%S", localtime()) + " **** Loading Models")

    predictions_frozen = []
    predictions_frozen, vect_out = Inference(path + "/pb/RNN_" + breath + "_" + quant + "_" + str(cases) + ".pb", X_valid)
    os.system("zip -r " + path + "/pb/compressed.zip " + path + "/pb/RNN_" + breath + "_" + quant + "_" + str(cases) + ".pb")
