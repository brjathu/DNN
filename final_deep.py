...
def export_model(sess_loc, pb_loc):
    ...
    new_saver = tf.train.import_meta_graph(sess_loc + "saved_rnn_100.ckpt.meta")
    new_saver.restore(sess, sess_loc + "saved_rnn_100.ckpt")
...
def Inference(filePath, X_valid):
    ...
    with tf.Session(config=config) as sess:
        ...
        pred_softmax = sess.graph.get_tensor_by_name("prefix/y_:0")
        # predict_fc = sess.graph.get_tensor_by_name("prefix/rnn/multi_rnn_cell_29/cell_1/dropout/mul:0")
        predict_fc = sess.graph.get_tensor_by_name("prefix/rnn/multi_rnn_cell_249/cell_1/dropout/mul:0")
...
def Inference_2(filePath, X_valid):
    ...
    with tf.Session(config=config) as sess:
        ...
        pred_softmax = sess.graph.get_tensor_by_name("y_:0")
        # predict_fc = sess.graph.get_tensor_by_name("prefix/rnn/multi_rnn_cell_29/cell_1/dropout/mul:0")
        predict_fc = sess.graph.get_tensor_by_name("rnn/multi_rnn_cell_249/cell_1/dropout/mul:0")
...
main_dir = "/flush1/sen040/final_models/deep/250/1.0/"
hidden_unit_folder = [32, 64]
breath = "breath_deep"
for h_folder in hidden_unit_folder:
    for layer in [2, 4, 6]:
        new_path = main_dir + str(h_folder) + "/" + str(layer) + "/100/"
        pb_location = "/flush1/raj034/DNN/final/rnn/deep/"
        model = str(h_folder) + "_" + str(layer)
        ...
        main_location = pb_location + model + "/"
        # predictions_frozen, vect_out = Inference_2(main_location + "model.pb", x_test)
        # os.system("zip -r " + main_location + "compressed.zip " + main_location + "model.pb")
        # logEntry("**** main file **** ")
        # logEntry(os.system("ls -l " + main_location + "compressed.zip"))
        # plotConfusionMatrix(list(predictions_frozen), y_test, 100, main_location, "confusion_matrix.png", "RNN - " + breath)

        # predictions_frozen, vect_out = Inference_2(main_location + "/model.pb", x_train)
        ...
        for quant in ["quant_no", "quant_yes"]:
            ...
            if(quant == "quant_yes"):
                ...
                # os.system("python converge_weights.py --min_n_weights 256 --file " + pb_location + model + "/model.pb" + " --save_location " + main_location + "pb/quantized_original.pb")
                ...
                # plotConfusionMatrix(list(predictions_frozen), y_test, 100, main_location, "confusion_matrix.png", "RNN - " + breath + "_" + quant)
            ...
            for lvl in [100, 80, 60, 40, 20]:

                if(quant == "quant_yes"):
                    lvl_x = min(math.ceil(h_folder * lvl / 100), h_folder)

                    # location = pb_location + model + "/quant_no/fact_" + str(lvl) + "/pb/RNN_" + breath + "_quant_no_fact_" + str(lvl_x) + ".pb"
                    # os.system("mkdir " + pb_location + model + "/" + quant)
                    ...
                    # # LOG_FILE = open(main_location +  'log.txt', 'a')
                    # os.system("python converge_weights.py --file " + location + " --save_location " + main_location + "/pb/RNN_" + breath + "_" + quant + "_fact_" + str(lvl_x) + ".pb")

                    # # accuracy test
                    # predictions_frozen = []
                    # predictions_frozen, vect_out = Inference(main_location + "/pb/RNN_" + breath + "_" + quant + "_fact_" + str(lvl_x) + ".pb", x_test)
                    # os.system("zip -r " + main_location + "/pb/compressed.zip " + main_location + "/pb/RNN_" + breath + "_" + quant + "_fact_" + str(lvl_x) + ".pb")
                    # logEntry(os.system("ls -l " + main_location + "/pb/compressed.zip"))
                    # plotConfusionMatrix(list(predictions_frozen), y_test, 100, main_location, "confusion_matrix.png", "RNN - " + breath + "_" + quant)

                    # predictions_frozen, vect_out = Inference(main_location + "/pb/RNN_" + breath + "_" + quant + "_fact_" + str(lvl_x) + ".pb", x_train)
                    # np.save(main_location + "d-vector.npy", vect_out)
                    ...
                    # # LOG_FILE = open(main_location +  'log.txt', 'a')
                    lvl_x = min(math.ceil(h_folder * lvl / 100), h_folder)
                    # build_pb_fact(pb_location + model + "/model.pb", main_location, breath, quant, lvl_x)
